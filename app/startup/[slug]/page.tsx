...
export default async function StartupPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const isBangaloreMapSource = "verificationStatus" in startup && startup.verificationStatus === "BANGALORE_MAP_SOURCE";
  ...
  return <main>
    ...
    <section className="profile shell">
      <div className="profileTop">
        ...
        <div><div className="eyebrow">{isBangaloreMapSource?"BENGALURU LOCATION-SOURCED PROFILE":isCommunitySource?"COMMUNITY-SOURCED REMOTE PROFILE":isTargetEmployer?"SDE TARGET EMPLOYER":isDirectorySource?"DIRECTORY-SOURCED PROFILE":"CURATED COMPANY PROFILE"}</div><h2>{startup.name}</h2><p>{startup.description}</p></div>
